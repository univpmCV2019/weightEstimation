...
def processDepth(dev,pbs,interval,dst,videoname):
    try:
        ...
        for i in range(1,depth_stream.get_number_of_frames()+1,interval):
            ...
            cv2.imwrite(dst + "/" +videoname+ str(frame_depth.frameIndex) + "_16bit.png",depth_array)
            cv2.imwrite(dst + "/" +videoname+ str(frame_depth.frameIndex) + "_8bit.png",depth_uint8)
        depth_stream.close()
        print("All depth frames extracted")
    except Exception as ex:
        print(ex)

def processColor(dev,pbs,interval,dst,videoname):
    try:
        ...
        for i in range(1,color_stream.get_number_of_frames()+1,interval):
            ...
            cv2.imwrite(dst + "/" +videoname+ str(frame_color.frameIndex) + "_color.png",color_array)
        color_stream.close()
